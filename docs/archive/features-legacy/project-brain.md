...
Before serialize/export: redact secret-like claim values (password/secret/token/api_key/private_key/credential markers and PEM private-key headers become `[REDACTED]`), sensitive path basenames (`[REDACTED_PATH]`), and never embed credential/env file contents. Ordinary source identifiers without those markers are left intact — redaction is heuristic, not a guarantee against every secret shape. Storage paths are constrained under the store root (including symlink realpath checks). Durable serialization zeroes non-deterministic `timingMs` fields.
